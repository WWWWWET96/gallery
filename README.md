...
| --- | --- |
| 09/19 | ■JEONGEUN
1. 데이터 구조 확립 및 ERD 추출
2. 서비스 로직의 구체화
    2.1 서비스는 크게 로그인과 낙찰로.. 두 가지??(미정) |
| 10/01 | 1. 로그인하면 상단우측 <LOGIN>에서 <000님> →누르면 작은 네비바 나와서 입찰내역, 장바구니, 내정보
2. 서치바 클릭 → 전체화면에 팝업처럼 서치박스 나오게. 백그라운드 블러처리.
3. 어드민 회원가입: 이메일, 아이디, 비밀번호
4. 유저 회원가입: 아이디, 비번, 이메일, 폰번호, 닉네임 
5. 어드민 페이지 추가→ 유저 회원관리(가입 승인, 회원 강퇴), 그림관리(그림 등록/수정/삭제)
6. 경매 끝나도 작품 남겨두고(완료상태), 아티스트페이지 수정 → 자음순으로 나열, 아티스트 선택하면 상단에 아티스트 소개/하단에 작품 |
| 10/2 | ■JEONGEUN  
1.toEntity는 service단에서 해결
→ 매개변수로 받는 user_id, art_id를 key 값으로 user ,art객체를 찾아서 service단에서 직접 toEntity(user, art)로 넣어주면 됨
2. 회원상태랑 휴면계정상태 두 속성으로 나누면 안되는 이유
→ 【비승인된 탈퇴된 회원】 이런건 애초에 말이 안되는 경우의 수임
3. enum은 객체인가
→ O
4. applicantDto로 처음 신청, 가격 수정(재신청) 다 가능하도록???
→user_id, art_id가 있어야 해당 데이터 찾아서 값 수정 가능하니까 
동일하게 이용해야한다고 생각… |
| 10/18 | ■JEONGEUN  
1. toEntity삭제 후 of로 dto↔entity convert 구현
2. 스프링시큐리티 로그인 수정 필요
→ loadUserByUsername()함수 삭제 후 jwt로 구현?
→ jwt을 사용하면 토큰을 통해 로그인하는듯
→JWT는 토큰 자체에 유저 정보를 담아서 암호화한 토큰
https://velog.io/@jkijki12/Spirng-Security-Jwt-%EB%A1%9C%EA%B7%B8%EC%9D%B8-%EC%A0%81%EC%9A%A9%ED%95%98%EA%B8%B0
https://inpa.tistory.com/m/entry/WEB-%F0%9F%93%9A-JWTjson-web-token-%EB%9E%80-%F0%9F%92%AF-%EC%A0%95%EB%A6%AC
3. 프로젝트에 대한 정책/api명세서 필요
4. 테스트 코드 작성 시,
→ @SpringBootTest : 무거움, 통합 테스트 시 이용
→ @DataJpaTest : 가벼움, 자체db만들어서 테스트 진행, repository테스트 시 유용
→ @Mock : Controller, Service의 단체테스트 진행 시 사용. 의존 빈을 직접주입하는 것이 아닌, mock의 형태로 주입하여 테스트 진행
→ test-resources-application.yml, db정보 없다면 main-resources-application.yml의 db정보 이용하여 db연결함 (테스트는 테스트용 db 사용하도록 지향. )
5. 유효성 검사는 프론트에서1차, 백에서2차 이뤄지는게 맞다고 봄
→ 맨 처음 컨트롤러로 들어올 때 dto에서 유효성 검사 이뤄지도록
→ entity에 들어올 때 유효성 검사 이뤄져도 똑같을듯? |
| 10/23 |  ■JEONGEUN
1. 반환값: 회원아이디(string) → 00님, pk아이디값/유저정보 (회원가입)
2. 유효성검사(프론트/백에서 한번씩): 회원가입, 그림 등록시간 현재보다 과거로 되지 않게 
4. api 명세서 작성
5. 회원가입, 로그인 화면단만들기 + 비동기통신 (통신해보기) |
| 11/1 | ■JEONGEUN
1. admin → 회원가입 시, admin_role입력(Manager) |
