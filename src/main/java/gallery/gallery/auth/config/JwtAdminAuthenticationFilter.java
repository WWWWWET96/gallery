...
@Component
public class JwtAdminAuthenticationFilter extends GenericFilterBean {
    //토큰을 검증하기위한 provider
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        //헤더에서 JWT를 받아온다.
        String token = jwtAdminTokenProvider.resolveToken((HttpServletRequest) request);
        //유효한 토큰인지 확인한다.
        if(token != null && jwtAdminTokenProvider.validateToken(token)){
            //토큰이 유효하면 토큰으로부터 유저 정보를 받아온다.
            Authentication authentication = jwtAdminTokenProvider.getAuthentication(token);
